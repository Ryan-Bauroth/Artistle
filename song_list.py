...
def get_artist_songs(artist_name):
    ...
    results = spot.search(q=artist_name, type='artist')     #gets the artists from spotify

    for item in results['artists']['items']:                #Adds the artist names to the list
        artist_list.append(item['name'])                

    
    results = spot.search(q="artist:"+artist_list[0], type='track', limit=50)   #gets 50 songs from artists that include 'given artist' in their name
